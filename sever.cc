...
private:
    ...
    // 发送帧到订阅者
    bool SendFrameToSubscriber(int client_id, const Frame& frame) {
        auto it = subscribers_.find(client_id);
        if (it == subscribers_.end() || !it->second.active) {
            return false;
        }

        if (!it->second.writer->Write(frame)) {
    ...
    void CleanupInactiveSubscribers() {
        ...
        for (const auto& [client_id, session] : subscribers_) {
            if (!session.active) {
        ...
        for (int client_id : inactive_clients) {
            ...
            std::cout << "Removed inactive client " << client_id << std::endl;
        }
    }
...
public:
    ...
    Status Subscribe(ServerContext* context, const SubscribeReq* request, ServerWriter<Frame>* writer) override {
        int client_id;
        {
            ...
            subscribers_[client_id] = {request->replay_seq_no_start(), writer, true};
            ...
            for (const auto& frame : sorted_frames) {
                if (frame.seq_no() >= request->replay_seq_no_start()) {
                    if (!SendFrameToSubscriber(client_id, frame)) {
        ...
        while (!context->IsCancelled() && !server_shutdown_) {
            cv_.wait(lock, [context, this]() { 
                return context->IsCancelled() || server_shutdown_; 
            });
        }
    ...
    Status Publish(ServerContext* context, ServerReader<Frame>* reader, Ack* response) override {
        ...
        while (reader->Read(&frame)) {
            ...
            for (auto& [client_id, session] : subscribers_) {
                if (session.active && frame.seq_no() >= session.replay_seq_no_start) {
                    if (!SendFrameToSubscriber(client_id, frame)) {
                        // 写入失败会在SendFrameToSubscriber中处理
                        continue;
                    }
                }
            }
