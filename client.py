...
def simulate_clients(num_clients=10):
    ...
    for i in range(num_clients):
        start_seq_no = i + 1
        end_seq_no = start_seq_no + 5  # 发送6个帧
        client = BroadcastClient(server_address='0.0.0.0:50051')
        frames = [client.create_frame(seq_no=j, payload=f"message {j}") for j in range(start_seq_no, end_seq_no + 1)]
        thread = threading.Thread(target=client.run, args=(start_seq_no, frames))
        thread.start()
